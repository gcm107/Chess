...
class EvaluateWorker:
    ...
    def load_current_model(self):
        """
        Loads the best model from the standard directory. If it doesn't exist, creates a new one.
        :return ChessModel: the model
        """
        model = ChessModel(self.config)
        if load_best_model_weight(model):
            print(f"Loaded best model")
        else:
            print(f"Creating new model")
            model.build()
            save_as_best_model(model)
            print(f"Saved new model as best model")
        
        if model.model is None:
            model.build()
        
        # Ensure the model has weights
        if not model.model.get_weights():
            raise ValueError("Model loaded but weights are empty")
